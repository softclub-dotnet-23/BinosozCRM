...
/**
 * Real backend data (GET /api/v1/dashboard/work-status) — deliberately kept separate from the
 * rest of DashboardPage, which is still mock data for Objects/Works/Payroll (different backend
 * modules, out of the current integration scope). Only Owner/Prorab get this card, matching the
 * endpoint's [Authorize(Roles = "Owner,Prorab")] — the query itself doesn't fire for other roles.
 */
export function WorkStatusFromBackendCard() {
  const { user } = useAuth();
  const isBackendSession = user?.isBackendSession ?? false;
  const allowed = isBackendSession && (user?.role === "owner" || user?.role === "prorab");
  const { data, isLoading, isError, error } = useDashboardWorkStatus();

  // A mock session (any role) never gets this card at all — it would otherwise silently show
  // stale/absent data with no indication a real backend login is what's actually missing. A real
  // backend session whose role just isn't Owner/Prorab (Brigadir/Accountant) also gets nothing,
  // matching the endpoint's own [Authorize(Roles = "Owner,Prorab")].
  ...
  return (
    <Card className="p-5 sm:p-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-ink">Статус работ (реальные данные backend)</h2>
      </div>

...
export function ProgressBar({ value, tone = "primary", className, trackClassName }: ProgressBarProps) {
  ...
  // existing width transition below only fired on later value *changes*, not on mount.
  ...
  return (
    <div
      ...
      aria-valuemin={0}
      aria-valuemax={100}
      className={cn("h-1.5 w-full overflow-hidden rounded-full bg-surface-4", trackClassName, className)}
    >
      <div

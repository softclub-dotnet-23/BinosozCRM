      ...
      <table className="w-full min-w-[520px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-border text-left text-xs text-ink-secondary">
            <th className="px-5 py-2.5 font-medium sm:px-6">{d.colObject}</th>
            <th className="px-3 py-2.5 font-medium">{d.colForeman}</th>
            <th className="px-3 py-2.5 font-medium">{d.colProgress}</th>
            <th className="px-3 py-2.5 font-medium">{d.colBudget}</th>
            <th className="px-3 py-2.5 pr-5 font-medium sm:pr-6">{d.colStatus}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-b border-border last:border-0 hover:bg-surface-1">

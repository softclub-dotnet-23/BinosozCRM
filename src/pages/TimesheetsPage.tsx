...
export default function TimesheetsPage() {
  ...
  const canApprove = isBackendSession && user?.role === "prorab";

  const [page, setPage] = useState(1);
  const { data, isLoading, isError, error, refetch } = useTimesheets({ page, pageSize: PAGE_SIZE }, isBackendSession);
  ...
  return (
    ...
    >
      {!isBackendSession ? (
        <BackendSessionRequired roleHint="Owner, Prorab или Brigadir" />
      ) : (
        <>

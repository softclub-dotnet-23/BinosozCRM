...
`Domain/Entities/` and `Infrastructure/Persistence/Configurations/` tree has one owner
regardless of which zone the entity's *feature* belongs to. If you're Shahrom and need an
entity for Zone B, don't run this yourself — ask Ahmad (or trigger `shahrom`).
...
3. Create the entity in `src/BrigadaCRM.Domain/Entities/`:
...
4. EF configuration in `src/BrigadaCRM.Infrastructure/Persistence/Configurations/`.

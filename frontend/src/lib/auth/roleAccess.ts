import type { UserRole } from "../../types";

/** Landing route right after login, and where a role gets bounced back to if it opens a route it can't use. */
export const ROLE_HOME: Record<UserRole, string> = {
  owner: "/dashboard",
  accountant: "/payroll",
  prorab: "/works",
  brigadir: "/brigades",
};

const FULL_ACCESS = "*" as const;

/**
 * Routes each role may open. An entry ending in "/*" allows that path and every
 * path nested under it (e.g. Prorab's "/brigades/*" also covers
 * "/brigades/composition" and "/brigades/assignments"). A plain entry (no "/*")
 * matches that exact path ONLY, so a role like Brigadir can be given its own
 * "/brigades" (their scoped crew view) without also inheriting the Prorab/Owner
 * composition and assignment management pages one level deeper.
 *
 * These lists mirror the backend controllers' [Authorize(Roles=...)] matrix —
 * the backend remains the real enforcement point; this only shapes navigation.
 */
const ROLE_ALLOWED_PREFIXES: Record<UserRole, string[] | typeof FULL_ACCESS> = {
  owner: FULL_ACCESS,
  // No "/dashboard": DashboardController is [Authorize(Roles="Owner,Prorab")].
  accountant: [
    "/payroll", "/payroll-entries", "/payroll-advances", "/absences", "/reports", "/budgets", "/estimates", "/objects", "/employees", "/profile",
  ],
  // NOTE: /payroll-advances (PayrollAdvancesController) is class-level
  // [Authorize(Roles="Owner,Accountant")] — Prorab and Brigadir are both excluded here, live-
  // verified via curl (both get a real 403), even though the GET method also carries a
  // "...,Brigadir" override that looks like it should admit Brigadir. It doesn't: ASP.NET Core
  // combines multiple [Authorize] attributes on one action with AND, not "most specific wins", so
  // Brigadir still fails the class-level check. Do not re-add either role without re-verifying live.
  prorab: [
    "/dashboard", "/objects", "/estimates", "/budgets", "/works", "/individual-tasks", "/timesheets", "/brigades/*", "/attendance", "/reports",
    "/material-requests", "/material-consumption-reports", "/material-deliveries", "/absences", "/payroll", "/work-orders", "/employees", "/profile",
  ],
  brigadir: [
    "/works", "/individual-tasks", "/timesheets", "/brigades", "/attendance", "/profile",
    "/material-requests", "/material-consumption-reports", "/payroll-entries", "/payroll", "/work-orders",
  ],
};

export const ROLE_LABEL: Record<UserRole, string> = {
  owner: "Владелец",
  accountant: "Бухгалтер",
  prorab: "Прораб",
  brigadir: "Бригадир",
};

export function isRouteAllowed(role: UserRole, pathname: string): boolean {
  const rule = ROLE_ALLOWED_PREFIXES[role];
  if (rule === FULL_ACCESS) return true;
  return rule.some((entry) => {
    if (entry.endsWith("/*")) {
      const base = entry.slice(0, -2);
      return pathname === base || pathname.startsWith(`${base}/`);
    }
    return pathname === entry;
  });
}

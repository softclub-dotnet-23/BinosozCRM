...
import { PayrollPeriodType, PieceworkDistributionMode } from "../services/companyApi";
...
export default function SettingsPage() {
  ...
  const SEARCH_INDEX: [string, SettingsTab][] = [
    ...
    [s.company.cardTitle, "company"], [s.finance.currency, "finance"], [s.finance.vatRate, "finance"],
      ...
      <div className="settings-page">
        ...
        <div className="settings-content-grid">
          <main className="settings-main">
            ...
            {tab === "company" && <CompanySettings s={s} />}
...
// GET/PUT /companies/current — the only Settings tab backed by a real endpoint. GET is any
// authenticated role; this page is Owner-only already (see roleAccess.ts), so PUT is always
// allowed here too.
function CompanySettings({ s }: { s: SettingsStrings }) {
  const c = s.company;
  ...
  const [draft, setDraft] = useState<{
    ...
    latenessGraceMinutes: string;
    latenessNotifyThresholdMinutes: string;
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      latenessGraceMinutes: String(company.latenessGraceMinutes),
      latenessNotifyThresholdMinutes: String(company.latenessNotifyThresholdMinutes),
  ...
  if (isLoading || !draft) {
    return <div className="settings-card-grid"><Card className="settings-card"><div className="settings-card-body"><p>Загрузка...</p></div></Card></div>;
  }
  ...
  function save() {
    ...
    updateMutation.mutate({
      pieceworkDistributionMode: draft.pieceworkDistributionMode,
      latenessGraceMinutes: Number(draft.latenessGraceMinutes) || 0,
      latenessNotifyThresholdMinutes: Number(draft.latenessNotifyThresholdMinutes) || 0,
  ...
  return <div className="settings-card-grid">
    <SettingsCard icon={Building2} title={c.cardTitle} saveLabel={updateMutation.isPending ? "Сохранение..." : s.save} onSave={save}>
      <TextRow label={c.companyName} value={draft.name} onChange={() => {}} readOnly="Название компании задаётся при создании" />
      <SelectRow
        label="Распределение сдельщины"
        value={String(draft.pieceworkDistributionMode)}
        onChange={(v) => setDraft({ ...draft, pieceworkDistributionMode: Number(v) as PieceworkDistributionMode })}
        options={[
          { value: String(PieceworkDistributionMode.Manual), label: "Вручную бригадиром" },
          { value: String(PieceworkDistributionMode.EqualAmongContributors), label: "Поровну между участниками" },
          { value: String(PieceworkDistributionMode.ProportionalToHours), label: "Пропорционально часам" },
        ]}
      />
      <TextRow
        label="Порог опоздания без штрафа, мин"
        value={draft.latenessGraceMinutes}
        onChange={(v) => setDraft({ ...draft, latenessGraceMinutes: v.replace(/\D/g, "") })}
      />
      <TextRow
        label="Порог уведомления об опоздании, мин"
        value={draft.latenessNotifyThresholdMinutes}
        onChange={(v) => setDraft({ ...draft, latenessNotifyThresholdMinutes: v.replace(/\D/g, "") })}
      />
      <SelectRow
        label="Период начисления зарплаты"
        value={String(draft.payrollPeriodType)}
        onChange={(v) => setDraft({ ...draft, payrollPeriodType: Number(v) as PayrollPeriodType })}
        options={[
          { value: String(PayrollPeriodType.Monthly), label: "Раз в месяц" },
          { value: String(PayrollPeriodType.SemiMonthly), label: "Два раза в месяц" },
        ]}
      />
      <TextRow label="Валюта по умолчанию" value={draft.defaultCurrency} onChange={(v) => setDraft({ ...draft, defaultCurrency: v.toUpperCase().slice(0, 3) })} />
    </SettingsCard>
...
function TextRow({ label, description, value, onChange, notImplemented, readOnly }: { label: string; description?: string; value: string; onChange: (value: string) => void; notImplemented?: string; readOnly?: string }) { return <div className="setting-row"><LabelText label={label} description={description} /><div><input className="setting-input" value={value} readOnly={Boolean(readOnly)} onChange={(event) => !readOnly && onChange(event.target.value)} /><NotImplementedNote text={notImplemented ?? readOnly} /></div></div>; }
...
// version/build/license/validUntil and the activity feed were hardcoded fake data with no
// backend concept behind them (no licensing system, no admin-audit-log read endpoint) — removed
// rather than left presenting fabricated values as real. Local storage usage is real, computed
// from the browser's own localStorage, so it stays.
function SystemInfo({ s }: { s: SettingsStrings }) {
  ...
  return <Card className="system-info-card"><header><Info size={16} /><h2>{i.title}</h2></header><div className="storage-label"><span>{i.storageLabel}</span><b>{storage} MB</b></div><div className="storage-bar"><span style={{ width: `${Math.min(100, Number(storage) * 2)}%` }} /></div></Card>;
}

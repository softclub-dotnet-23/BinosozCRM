...
interface DonutChartProps {
  ...
  /** Enables the chart segment animation. */
  animate?: boolean;

  /** Animates the numeric value displayed in the center. */
...
export function DonutChart({
  ...
  const summary =
    ...
    `${centerLabel}: ${centerValue}. ${data
      .map(
        (entry) =>
          `${entry.category} — ${valueFormatter(entry.amount)}`,
      )
      .join(", ")}`;
  ...
  return (
    ...
    >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              strokeWidth={2}
              isAnimationActive={
                animate && chartAnim.isAnimationActive
              }
      ...
      >
        ...
        >
          <p className="text-xs leading-tight text-ink-secondary">
            {centerLabel}
          </p>

          <p className="mt-1.5 break-words text-lg font-bold leading-[1.2] tabular text-ink">
            {typeof animatedCenterValue === "number" ? (
              <AnimatedNumber value={animatedCenterValue} />
            ) : (
              centerValue
            )}
          </p>

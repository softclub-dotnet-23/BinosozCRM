...
/** The four roles the backend actually issues (Domain/Enums/Role.cs) — nothing else can log in. */
export type UserRole = "owner" | "prorab" | "brigadir" | "accountant";
...
/** Safe, non-sensitive subset of the authenticated user persisted in the session
 * (no status, no password). Always backed by a real backend JWT session. */

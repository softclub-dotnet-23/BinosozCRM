...
SignalR-хаб `/hubs/work-orders` (Api/Hubs/WorkOrdersHub.cs) — группа `company:{companyId}` из JWT-claim, JWT передаётся через query-string `access_token` (браузер не может выставить заголовок на WebSocket-handshake). Реально реализованные события (Api/Realtime/*.cs): `WorkOrderStatusChanged`, `WorkOrderOverdue`, `IndividualTaskOverdue`, `MaterialShortageReported`. `AttendanceMarked` из более ранних заметок не реализован — не описывать его как рабочий. Подключён в `frontend/src/hooks/useWorkOrdersHub.ts` (единое соединение на весь защищённый роут, инвалидирует те же React Query ключи, что и соответствующая страница).

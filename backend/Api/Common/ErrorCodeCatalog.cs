...
//
// USER_PHONE_ALREADY_EXISTS (frontend integration, GET/POST /users): not in
// §9.2's table — Phone has a UNIQUE index (§6) but no endpoint wrote a second
// User row until now (§5.2's "Owner заводит [остальных] через API" was never
// backed by a POST /users controller before this pass). A repeat phone reads
// as 409, same family as PRORAB_ALREADY_ASSIGNED/CONCURRENCY_CONFLICT — an
// existing-state conflict, not a missing row or a validation failure.
//
...
public static class ErrorCodeCatalog
{
    private static readonly Dictionary<string, int> StatusCodesByCode = new()
    {
        ...
        ["USER_PHONE_ALREADY_EXISTS"] = StatusCodes.Status409Conflict,

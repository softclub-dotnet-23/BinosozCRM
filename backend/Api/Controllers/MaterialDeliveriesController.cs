using Api.Common;
using Api.Contracts.MaterialDeliveries;
...
// MASTER §9.4: "GET,POST /material-deliveries Prorab+" — no Brigadir role
// at all here, unlike the other two material entities.
[ApiController]
...
public sealed class MaterialDeliveriesController(ISender sender) : ControllerBase
{
    ...
    public async Task<IActionResult> Create(CreateMaterialDeliveryRequest request, CancellationToken cancellationToken)
    {
        var command = new CreateMaterialDeliveryCommand(
            request.ObjectId, request.MaterialRequestId, request.MaterialName, request.Unit,
            request.Qty, request.UnitCost, request.SupplierName);

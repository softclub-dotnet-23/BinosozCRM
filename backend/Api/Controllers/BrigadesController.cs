...
public sealed class BrigadesController(ISender sender) : ControllerBase
{
    ...
    [HttpPut("{brigadeId:guid}/active")]
    [Authorize(Roles = "Owner,Prorab")]
    public async Task<IActionResult> SetActive(Guid brigadeId, SetBrigadeActiveRequest request, CancellationToken cancellationToken)
    {
        var result = await sender.Send(new SetBrigadeActiveCommand(brigadeId, request.IsActive), cancellationToken);
        return result.ToActionResult(HttpContext);
    }
}

...
**Обновление 2026-07-31:** это решение по факту отменено в репозитории — коммиты
`955e4d5` (удаление frontend-scope) → `b4f52d4` (возврат папки) → `1a1419d`
(добавление полноценного React+Vite+TS фронтенда с mock-данными) уже лежат в
истории. С 2026-07-31 ведётся отдельный трек интеграции этого фронтенда с
реальным backend (`docs/PROGRESS.md`, ветка
`feature/shahrom-frontend-backend-integration`, по модулю за чекпоинт).
Историческое обоснование выше не переписывается — просто фиксируется факт
разворота, чтобы код и документ не расходились молча.

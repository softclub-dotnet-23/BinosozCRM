...
**Один файл на весь проект: backend, frontend, design, Telegram-бот.** Свёрнуты `Brigadir1.zip` (4 промта), `Brigadir2.zip` (12 docs), ревью пробелов (11 пунктов) и всё, что найдено при финальном разборе. Это спецификация, не код — по ней пишет человек или AI-агент, шаг за шагом.
...
**Три поверхности, один backend:**

| Поверхность | Кто | Почему именно так |
|---|---|---|
| Веб-панель (React) | Owner, Prorab, Accountant | Десктоп, много данных, таблицы, отчёты |
| Telegram-бот | Brigadir (и рабочие через него) | Объект = перчатки, солнце, плохой интернет. Telegram уже стоит, переживает разрывы связи, интерфейс знаком. SPA там — плохой инструмент |
| REST API + SignalR | Контракт между ними | Один Application-слой, два входа |
...
- **Переработка — сознательно вне MVP.** Не потому что не нужна, а потому что **нет данных**: есть `ShiftStartTime`, нет `ShiftEndTime` и нет нормы часов. Считать переработку не от чего. Сначала — `ShiftEndTime` + норма в Phase 3, формула — отдельным решением после. Зафиксировано в §17 как открытый вопрос (№6), не забыто.
...
Панель, по расписанию (2 раза в месяц). Формирует черновики зарплаты, проверяет расшифровку (вычеты/премии/авансы), утверждает, проводит оплату. Выдаёт авансы. Единственный кроме Owner, кто видит `PayRate` всех рабочих. **Не** управляет производством, **не** подтверждает премии (это решение прораба по факту работы).
...
трёх Owner заводит их в панели (`POST /users`), с паролем-приглашением и тем же
...
Единственный источник правды по формулам. Ни одна из них не дублируется в React или в Telegram-хендлере — фронт и бот показывают готовый результат от backend, не считают сами.
...
Один `Worker` — один `PayRateType`. Если бригада делает и почасовые, и сдельные работы — это разные `Worker`-записи или смена `PayRateType` (что меняет расчёт **с даты изменения**, не задним числом). Историю ставок в MVP не храним — открытый вопрос §17 №7 (нужна ли `WorkerPayRateHistory`).
...
Hard `400 WORKER_UNDERAGE`, не warning, не «сохранить всё равно». На бэкенде — граница безопасности. На фронте — дублируется как мгновенная подсказка при вводе (показывает вычисленный возраст).

**Граничный случай:** дата рождения неизвестна точно — система **не** разрешает пропустить поле. Смягчение (приблизительная дата с пометкой) — бизнес-решение, §17 №13.
...
Объект: перчатки, солнце, нестабильный интернет. Telegram уже установлен, переживает разрывы, интерфейс знаком. Это архитектурное решение, не экономия. Бот — **основной интерфейс бригадира**, не приложение к панели.
...
Не подтверждает премии, не принимает/отклоняет наряды, не выдаёт авансы, не создаёт `AbsenceRecord` — всё это требует контекста панели и другой роли.

### 10.6 Язык ← пробел №11

Бот — **ru + tg** (таджикский), переключение `/language`, хранится в `TelegramLink.PreferredLanguage`. Компания в Таджикистане, бригадир — не обязательно свободно читает по-русски, а бот это его единственный интерфейс. Панель (Owner/Prorab/Accountant, офисные роли) — ru-RU, этого достаточно для MVP. Ресурсы — `.resx`, не строки в коде.
...
Для Owner/Prorab/Accountant панель — **единственный** вход. Забыл пароль = потерял доступ к системе, где считается зарплата.
...
POST /auth/forgot-password { phone }
    ...
    → доставка (в порядке приоритета):
        ...
        2. Нет TelegramLink → ручной сброс через Owner в панели (тот же экран, где
           создаются пользователи) — тоже бесплатно, но требует, чтобы кто-то с
...
**CORS** — allow-list ровно на домен панели. **Никогда** `AllowAnyOrigin` вместе с credentials.
...
- **Доступ:** полный номер документа виден только Owner/Accountant. Prorab видит **маскированный** (`****4567`) — ему для приёмки работы полный не нужен. Brigadir — не видит вообще. Маскирование — на уровне **Response DTO** (разные DTO под разные роли), не CSS на фронте.
...
- **Шифрование на уровне колонки** (`DocumentType`/`DocumentExpiryDate`) — **вне MVP**, зафиксировано осознанно: при одной компании и одной БД шифрование колонки защищает только от того, у кого уже есть доступ к базе, то есть почти ни от чего, а ключи усложняют бэкапы. Пересмотреть при появлении внешнего хостинга БД. §17 №10.
...
# ЧАСТЬ V. FRONTEND И DESIGN

## 13. Frontend — React-панель (Owner / Prorab / Accountant)

Бригадир сюда **не заходит** — у него бот (§10). Экраны для него здесь не проектируются.

### 13.1 Стек

| Что | Чем | Правило |
|---|---|---|
| Каркас | React 18 + Vite + TypeScript | |
| Роутинг | React Router, protected routes по роли из JWT | недоступный роли маршрут — **редирект**, не показ с последующей 403 |
| Серверный стейт | TanStack Query | весь. Кэш, инвалидация после мутаций |
| UI-стейт | Zustand | **только** UI: тема, sidebar. Серверные данные сюда не дублировать |
| Стили | Tailwind + shadcn/ui (Radix) | |
| Формы | react-hook-form + zod | zod-схемы зеркалят FluentValidation теми же числами |
| Drag&drop | @dnd-kit | Kanban, **оптимистично** с откатом |
| Real-time | @microsoft/signalr | `invalidateQueries` на конкретный ключ, не полный рефетч |
| Графики | Recharts | |
| Анимации | Framer Motion | экономно: смена статуса, модалка. Не декор |
| HTTP | Axios + JWT-интерцептор | refresh на 401, один retry, дальше — логин |
| Типы | openapi-typescript | из Swagger, **не** руками |

### 13.2 Layout (уровень plane.so)

- **Sidebar** сворачиваемый: переключатель компании (заглушка), навигация — Дашборд, Объекты, Бригады, Наряды и задачи, Явка, Материалы, Зарплата (Accountant/Owner), Пользователи (Owner).
- **Топбар**: хлебные крошки, поиск, переключатель представления, аватар/тема.
- **Правая выезжающая панель** — детали наряда/задачи. Не отдельная страница — не терять контекст списка.
- **Command palette (Cmd/Ctrl+K)** — поиск по объектам/нарядам/бригадам + быстрые действия.

### 13.3 Наряды и задачи — 4 представления

`WorkOrder` и `IndividualTask` **вместе**, различаются иконкой типа — это комбинированный статус (§8.6) в UI, не два раздела.

- **Board (Kanban)** — колонки по статусу, drag&drop → API. Backend отклонил переход → карточка **откатывается с анимацией**, не исчезает молча.
- **List** — группировка по объекту/бригаде/сроку. Просроченное — отдельная группа наверху.
- **Calendar** — по `DueDate`/`DueAt`.
- **Table** — все поля, экспорт CSV (Accountant/Owner).

Карточка: `Code` (моно, вторичный) → заголовок (одна строка, `...`) → статус-бейдж + срок (просрочен — `#DC2626`) → аватар-инициалы. Иконка премии в углу: серый контур = черновик, зелёный залитый = подтверждена — деньги в процессе видны, не открывая карточку.

### 13.4 Экраны с бизнес-логикой

**Дашборд** — счётчики (объекты, на проверке, просрочено, заявки) + графики: закрытые наряды по неделям, топ материалов по частоте нехватки.

**Объекты** — CRUD, смета, **бюджет vs факт** с разбивкой (`/objects/{id}/cost-breakdown`, §8.10): материалы + зарплата отдельными строками, пометка «зарплата — за закрытые периоды». Не одна цифра, которая врёт.

**Бригады** — CRUD, состав. `BirthDate` с валидацией 18+ **на клиенте**: меньше 18 на `HireDate` → поле красное с точным возрастом («16 лет 4 месяца на дату найма»), сабмит недоступен. Не ошибка после отправки.

**Наряд/задача — правая панель** — прогресс `Σ ReportedQty / PlannedQty`, лента отметок с фото (последнее сверху). Accept/Reject **не рендерятся** вне `OnReview` (отсутствуют в DOM, не скрыты CSS). Reject → модалка, причина обязательна.

**Блок распределения сдельщины** (§1.1) — таблица: рабочий, %, сумма. Прораб видит, что предложил бригадир, может поправить перед подтверждением. Сумма процентов — большая цифра, красная при ≠100%.

**Блок премии** — сумма от бригадира, поле правки, кнопка подтверждения, под ней серым: «не войдёт в зарплату без подтверждения».

**Явка** — таблица по объекту/периоду. `LateMinutes` — иконка часов + тултип («пришёл 8:25, план 8:00, опоздание 25 мин»), **не** голое число. `EnteredManually` — отдельная иконка. Приёмка построчно/пакетом. Дни с `AbsenceRecord` — своим цветом, не пусто (пусто = прогул, это разные вещи).

**Материалы** — два потока **рядом** (двухколоночно): слева лента дневных отчётов, справа очередь заявок. Материал с нехваткой 2+ раза за 7 дней — бейдж «повторяется». Заявка показывает `QtyDelivered / Qty` прогресс-баром — частичная поставка видна.

**Зарплата** — строка рабочего сворачиваемая. Развёрнутая = **чек**:
```
  + CalculatedAmount        7 040.00   → клик: часы/наряды
  − LatenessDeduction          43.33   → клик: дни с опозданиями
  + Bonus                     200.00   → клик: за какие задачи
  − AdvanceDeducted         3 000.00   → клик: когда выдан, кем
  ± Adjustment                  0.00   (причина текстом)
  ─────────────────────────────────
  = FinalAmount             4 196.67
```
Отрицательный `FinalAmount` — **красным, с пояснением** «аванс превысил заработок», не спрятать. Approve → Paid — модалка с суммой прописью, необратимо.

**Пользователи** (Owner) — CRUD, роли, назначение прорабов на объекты, привязка Telegram-кодов.

### 13.5 Правила

- SignalR обновляет счётчики без перезагрузки.
- Числа/даты — `ru-RU` (запятая, дд.мм.гггг), валюта — `TJS`.
- Роль в UI — удобство, **не** граница безопасности. Граница всегда на backend.

---

## 14. Design

Референс — **plane.so**: минимализм, воздух, тонкие границы, никаких тяжёлых теней. Референс на уровень качества, не на копирование бренда.
...
|---|---|---|
| Фон | `#FFFFFF` | `#0D0E11` |
| Поверхность | `#FAFAFA` | `#17181C` |
| Границы (1px) | `#E5E7EB` | `#26282E` |
| Текст | `#1A1A1A` | `#E8E8E8` |
| Текст вторичный | `#6B7280` | `#9CA3AF` |
| Акцент | `#4F46E5` | `#6366F1` |

**Статусы** (наряды и задачи — единая палитра): New `#6B7280` · Assigned `#8B5CF6` · InProgress `#3B82F6` · OnReview `#F59E0B` · Accepted/Done `#10B981` · Rejected `#EF4444` · Closed `#6B7280` + иконка замка.

**Опоздание/просрочка** — `#DC2626` + иконка часов. **Не заливка** карточки — иначе спорит по смыслу с красным статусом Rejected.

**Отсутствие** (больничный/отпуск) — `#0EA5E9` (голубой), визуально **не** тревожный: человек не виноват, что болел. Отличается и от опоздания (красный), и от прогула (серый).

Тёмная тема — **не инвертированная** светлая: статус-бейджи подобраны отдельно под контраст на тёмном.

### 14.2 Типографика

**Inter**. Заголовки 600–700, тело 400–500. Шкала: 12 / 13 / 14 / 16 / 20 / 24 px — дисциплина шкалы важнее конкретных цифр. Интерлиньяж ~1.5, у заголовков лёгкий отрицательный трекинг.

Цифры — **табличные** (`font-variant-numeric: tabular-nums`) везде, где деньги: в столбце суммы должны выравниваться по разряду, иначе чек не читается.

### 14.3 Экраны — визуальные решения

**Явка** — `LateMinutes` = иконка + тултип. Строка с опозданием — **тонкая левая полоса** `#DC2626`, не заливка (заливка = «ошибка/удаление», другое сообщение).

**Материалы** — двухколоночно. «Повторяется» — жёлтый `#F59E0B`, тот же тон, что OnReview: оба = «требует внимания». Осознанное родство.

**Премия** — выделенная карточка внутри панели, не строка формы: сумма крупно, кнопка акцентная, под ней мелким серым предупреждение.

**Распределение сдельщины** — таблица с процентами, сумма процентов **крупно**: зелёная при 100%, красная иначе. Это единственная цифра, которая блокирует отправку — она должна быть заметнее остальных.

**Зарплата** — как квитанция: моноширинные цифры, знаки `+`/`−` цветом, жирная черта, итог крупным.

### 14.4 Состояния

- **Пустой список** — не «нет данных», а призыв («Создать первый наряд»).
- **Загрузка** — skeleton-карточки, не спиннер на весь экран.
- **Ошибка сети** — тост с retry, не блокирующий модал.
- **Оптимистичный drag&drop** — карточка едет сразу, при отказе сервера откат с лёгкой тряской.

### 14.5 Что сдать

Токены (готовые лечь в Tailwind config) + ключевые экраны в высокой детализации: Kanban, правая панель деталей (с блоками премии и распределения), Дашборд, Зарплата с развёрнутой расшифровкой. Light + dark минимум для Kanban.
...
**Phase 0 — Foundation.** Solution (4 слоя), MediatR + FluentValidation + `Result<T>`, `Company` (первая сущность — от неё зависят все `CompanyId`), `User`, JWT + refresh **с ротацией**, Argon2id, rate limiting на логин **сразу** (не «потом»), `/health`, авто-миграция при старте, React-каркас + логин, CI (build + test + vulnerable-scan, zero warnings).
*DoD:* логин работает, роль в токене, `/health` отвечает, CI зелёный.

**Phase 1 — Объекты и бригады.** `Customer`, `ConstructionObject`, `EstimateItem`, `Brigade`, `Worker` (18+, `ShiftStartTime`, `UserId`), `ProrabObjectAssignment`, `AdminAuditLog`. React: CRUD-экраны по `docs/Design.md` (не временная вёрстка).
*DoD:* прораб создаёт объект, бригаду, назначает бригадира, добавляет рабочих; 18+ блокирует.

**Phase 2 — Наряды и задачи (ядро).** `WorkOrder`, `IndividualTask`, `TaskLog`, `Code`, state machines, `WorkOrderProgress`, upload фото, SignalR. **Telegram-бот v1**: привязка (TTL+хеш), secret_token, идемпотентность, «Моя бригада», «Мои наряды». React: 4 представления, Kanban, правая панель.
*DoD:* прораб создал наряд → бригадир в боте отметил и отправил → прораб принял в панели → статус обновился у обоих в реальном времени.

**Phase 3 — Явка, задачи, отсутствия.** `Timesheet` + `LateMinutes`, `AbsenceRecord`, `IndividualTask` полностью в боте, `CompletedEarly` + черновик премии, вечернее напоминание. React: экран Явка.
*DoD:* явка бригады через бот, опоздания считаются, больничный не даёт вычета.

**Phase 4 — Материалы.** `MaterialRequest` (+`QtyDelivered`, `PartiallyDelivered`), `MaterialConsumptionReport`, `MaterialDelivery`, авто-переход по частичным поставкам, `MaterialShortageReported`. Бот: «Материалы». React: два потока.
*DoD:* отчёт → заявка одним действием → прораб согласовал → частичная поставка отражена.

**Phase 5 — Зарплата.** `WorkOrderPayoutShare` (+ бот-флоу распределения), `PayrollEntry` со **всеми** слагаемыми (§8.0, 8.1, 8.7, 8.8), `PayrollAdvance`, фоновая задача черновиков, подтверждение премий, `/objects/{id}/cost-breakdown` (§8.10). React: экран Зарплата с чеком.
*DoD:* бухгалтер видит черновик с полной расшифровкой, аванс удержан, факт по объекту включает ФОТ.

**Phase 6 — Полировка и запуск.** `GET /dashboard/work-status`, Kanban на дашборде, фоновая просрочка, уведомления в Telegram всем ролям, `tg` в боте, бэкапы + **проверка восстановления**, мониторинг/алерты, **полный security-review (§11) и пентест — до первого реального использования на деньгах**.
...
- **Бот — единственный канал бригадира.** Нет Telegram = блокер для бригады. Fallback: прораб может отметить явку/выполнение за бригадира через панель, с `EnteredManually = true`. Это не костыль — это ровно то, что происходило до системы (прораб записывал в тетрадь), только теперь с аудитом. §17 №9 — решить, достаточно ли.
...
## 17. Открытые вопросы — решать заказчику, не выдумывать
...
| 9 | Fallback без Telegram у бригадира | прораб отмечает через панель с `EnteredManually` |
| 10 | Шифрование PII на уровне колонки | вне MVP, пересмотреть при внешнем хостинге БД |
| 11 | Язык панели | ru-RU (бот — ru+tg) |
...
## 18. Что закрыто этим документом

**Из ревью (11 пунктов):** ①CalculatedAmount §8.0 · ②webhook secret+идемпотентность §10.3 · ③аванс §8.8 · ④больничный/увольнение §8.9 · ⑤прорабы по объектам §1.2 · ⑥частичные поставки §8.2 · ⑦переработка — обоснованно вне MVP §17.6 · ⑧fallback без Telegram §16 · ⑨сброс пароля §11.2 · ⑩факт по объекту §8.10 · ⑪health/бэкапы/CI/язык/PII §11.6, §11.8, §10.6.
...
**Свёрнуто из:** `Brigadir1.zip` (PROMPT_Backend/Frontend/Design/Telegram), `Brigadir2.zip` (12 docs), ревью пробелов, все решения из истории обсуждения.
